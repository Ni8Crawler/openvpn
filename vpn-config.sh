...
if [ -n "$srvname" -a -z "$clntname" ]; then
    ...
    [ -f $srvname.conf ] || (gzip -d </usr/share/doc/openvpn/examples/sample-config-files/server.conf.gz |
    sed -e "s,;local a.b.c.d,local $srvname," \
        ...
        > $srvname.conf && chmod 600 $srvname.conf && echo "Made $srvname.conf")
